...
- Runtime marker is `2.0.0`. PR #9 merged the final Version 2.0.4 foundation, and the formal `v2.0.0` release changes no feature behavior. Production deployment remains a separate task; automatic application and automatic email are not implemented. Interview Center, Mock Interview, formal STAR Stories, browser extension, and multi-model routing move to later versions.
