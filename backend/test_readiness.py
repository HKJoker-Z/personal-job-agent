...
class ReadinessTest(unittest.TestCase):
    ...
    def test_health_is_lightweight_and_version_1_9(self):
        with temporary_test_database(), tempfile.TemporaryDirectory() as directory:
            self.configured_runtime(directory)
            from main import health_check

            self.assertEqual(health_check(), {"status": "ok", "service": "personal-job-agent", "version": "1.9"})

    def test_readiness_version_is_1_9(self):
        with temporary_test_database(), tempfile.TemporaryDirectory() as directory:
            config, _ = self.configured_runtime(directory)
            payload, _status = readiness_status(config)
            self.assertEqual(payload["version"], "1.9")
