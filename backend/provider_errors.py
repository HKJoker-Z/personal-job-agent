...
def classify_provider_exception(exc: BaseException) -> str:
    ...
    # A concrete HTTPX/network cause is a transport failure, but not a
    # connection timeout. APIConnectionError itself is intentionally not
    # enough evidence: OpenAI uses it for every non-timeout transport error.
    if any(
        isinstance(item, (httpx.TransportError, ConnectionError, OSError))
